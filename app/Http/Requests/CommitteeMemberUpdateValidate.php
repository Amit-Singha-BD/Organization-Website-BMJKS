...
class CommitteeMemberUpdateValidate extends FormRequest
{
    ...
    {
        // Route-model binding থেকে member id নেওয়া
        $memberId = $this->route('committeeMember') ? $this->route('committeeMember')->id : null;
        ...
        return [
            ...
            'role' => [
                ...
                Rule::unique('committee_members')
                    ->where(fn($query) => $query->where('CommitteeYear_id', $this->CommitteeYear_id))
                    ->ignore($memberId), // update করার সময় নিজের id ignore করবে
            ],
