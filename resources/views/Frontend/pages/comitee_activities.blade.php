...
@section('content')
    ...
    <div class="container mb-5">
        <div class="tab-pane fade show active" id="notice" role="tabpanel" aria-labelledby="notice-tab">
                        ...
                        <div class="col-12 col-lg-4">
                            <form action="{{ route('committee.activities.search') }}" method="get">
                                ...
                                <div class="input-group">
                                    <input type="text" name="search" class="form-control" value="{{ request()->search }}" placeholder="কার্যক্রমের নাম লিখুন..." aria-label="Search">
            ...
            <!-- Search and Filter Result Section -->
            @if(Route::is('committee.activities.search') || Route::is('committee.activities.filter'))
                <section class="py-3">
                    <div class="container">
                        @if($activities->isNotEmpty())
                            @foreach($activities as $activitie)
                                <div class="activity-card" data-category="activity" data-year="{{ $activitie->committee_year_id }}">
                                    <div class="activity-header">
                                        <div class="row align-items-center">
                                            <div class="col-md-3 col-12">
                                                <div class="d-flex justify-content-around activity-section">
                                                    @foreach ($activitie->committeeMembers as $member)
                                            ...
                                            <div class="col-md-9 col-12">
                                                @foreach($activitie->committeeYears as $committeeYear)
                                            ...
                                            <div class="col-md-3 col-12">
                                                <div class="d-flex justify-content-around activity-section">
                                                    @foreach ($committeeYear->committeeMembers as $member)
                                                        <div class="text-center">
                                                            <img src="{{ asset($member->photo) }}" class="comitee-profile-img rounded-circle" alt="{{ $member->name }}">
                                                            <div class="profile-name">{{ $member->name }}</div>
                                                            <div class="profile-title">
                                                                {{ $member->role == 1 ? 'সভাপতি' : ($member->role == 4 ? 'সম্পাদক' : '') }}
                                                            </div>
